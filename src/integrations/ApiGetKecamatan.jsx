...
export class GetKecamatanAPIRequest{

    fetch = async () => {
        let url = process.env.REACT_APP_HOST_VOT + '/getkecamatan/get';
        let request = {
            method: 'GET',
            // headers: {
        ...
        if (response) data = await (new GetKecamatanAPIResponse(response)).getData();
        return { response, data };
    };

}

export class GetKecamatanAPIResponse extends APIResponse {
    validateResponse = () => {
        let error = ErrorMessage;
